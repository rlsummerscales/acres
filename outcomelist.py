...
class OutcomeList:
  ...
  def computeStatistics(self, errorOut):
    ...
    stats = {}
    stats['outcome'] = countMatches(aOutcomeTemplates, self.outcomeTemplates, errorOut)
...
#         epNode.appendChild(self.getGroupXML(doc, omTemplate, idPrefix))
  ...
  def writeOutcomeEvaluationForm(self, template, out, primary=False):
    writeEvaluationElement(template.getCanonicalName(), out)
    if primary:
      writeEvaluationElement('type: primary', out, indentLevel=1)
      
    for ssTemplate in template.summaryStats:
      out.write(bullet(1)+'Endpoint:\n')
      out.write(bullet(2)+'Less effective:'+ ssTemplate.lessEffective.getGroup().name +': ')
      out.write(ssTemplate.lessEffective.statisticString())
      out.write('\n')
      out.write(bullet(2)+'More effective:'+ ssTemplate.moreEffective.getGroup().name +': ')
      out.write(ssTemplate.moreEffective.statisticString())
      out.write('\n')
      rLabel = ssTemplate.riskType()
      nnLabel = ssTemplate.numberNeededType()
  
      out.write(bullet(2)+rLabel+ ': ' + ssTemplate.arrString())
      if ssTemplate.hasConfidenceIntervals():
    ...
    for omTemplate in template.unusedNumbers:
      if omTemplate.getGroup() != None:
        if omTemplate.getTime() != None:
          ...
          time = ''
        out.write(bullet(1)+'Endpoint:\n')
        out.write(bullet(2)+ omTemplate.getGroup().name + ': ' )
  ...
  def writeEvaluationForm(self, out):    
    ...
    writeElementsMissing('outcomes', out)

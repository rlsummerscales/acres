...
class OutcomeNumber(BaseQuantityTemplate):
  ...
  def __init__(self, token):
    ...
    BaseQuantityTemplate.__init__(self, token, 'on')    
  ...
  def correctAssociation(self, mType):
    ...
    if mType == 'group':
      ...
      print 'Error: invalid mention type in OutcomeNumber.correctAssociation'
      print 'mType:', mType
      sys.exit(1)
      return -2
...
#     """ return a string containing outcome number and statistics """
#     if self.getGroupSize() > 0:
#       return self.eventRateString() + ' ' + str(self.getOutcomes()) \
#               + '/' + str(self.getGroupSize())  
#     else:

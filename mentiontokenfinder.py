...
class MentionTokenFinder(BaseMentionFinder):
  ...
  def train(self, absList, modelFilename):
    ...
    featureFilename = 'features.token.'+self.entityTypesString+'.train.txt'
  ...
  def test(self, absList, modelFilename):
    ...
    featureFilename = 'features.token.'+self.entityTypesString+'.test.txt'
...
#    self.cleanupMentions(absList)
  ...
  def tokensToClassify(self, sentence):
    ...
    for token in sentence:
      if token.pos[0] == 'V' or token.pos[0] == 'N':
        list.append(token)
    return list
...
#          features.add(prefix+'gov_token_'+govToken.lemma)
      ...
      features = features.union(self.lexicalFeatures(govToken, prefix))
